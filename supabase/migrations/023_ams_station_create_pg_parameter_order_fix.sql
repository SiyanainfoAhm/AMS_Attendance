-- Fix: 022 used invalid PG parameter order (defaulted arg before required code/name), so CREATE failed.
-- Re-create station create with valid signature: (text, text, text, uuid, uuid)
...
begin
  for fn in
    ...
    where n.nspname = 'public'
      and p.proname = 'ams_sp_company_station_create'
  loop
